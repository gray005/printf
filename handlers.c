 ...
 */
int handle_wite_char(char c, char buffer[], int flags, int width, int precision, int size)
{ /* char is stored at left and paddind at buffer's right */
	...
	if (width > 1)
	{
		...
		if (flags & F_MINUS)
			return (write(1, &buffer[0], 1) + write(1, &buffer[0], 1));
	}
...
int write_number(int is_negative, int ind, char buffer[], int flags, int width, int precision, int size)
{
	...
	return (write num(ind, buffer, flags, width, precision, length, padd, extra_ch));
}
...
int write_num(int ind, char buffer[], int flags, int width, int prec, int length, char padd, char extra_c)
{
	...
	if (width > lenth)
	{
		...
		else if (!(flags & F_MINUS) && == '0')/* extra char to left of padd */
		{
			if (extra_c)
...
{
	/* The number is stored at the buffers's right and starts at position i */
